...
def calculate_progress(tasks: list[SprintTask]) -> SprintProgress:
    ...
    completed_points = sum(task.points for task in tasks if task.status == SprintTaskStatus.DONE)
...
def _project_status(tasks: list[SprintTask]) -> ProjectSprintStatus:
    ...
    if all(task.status == SprintTaskStatus.DONE for task in tasks):
        return ProjectSprintStatus.COMPLETE
    if any(task.status == SprintTaskStatus.BLOCKED for task in tasks):
...
class SprintService:
    ...
    def load_sprint(self) -> SprintSummary:
        ...
        needs_attention = [
            task for task in tasks if task.status == SprintTaskStatus.BLOCKED
        ] or [
    ...
    def _project_summary(self, project: ProjectConfig, tasks: list[SprintTask]) -> ProjectSprintSummary:
        return ProjectSprintSummary(
            ...
            blockers=[task for task in tasks if task.status == SprintTaskStatus.BLOCKED],
            tasks=tasks,
            progress=calculate_progress(tasks),
